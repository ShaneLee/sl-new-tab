...
function fetchFiles(bucket, tags, onLoaded) {
  if (bucket) {
    category = bucket
  }
  ...
  return api(filesTagsEndpointFn(category, currentTags, page, size), {
    method: 'GET',
    headers: headers,
  })
    .then(response => {
...
function goToMedia(index, direction) {
  if (index < 0) {
    if (!isFirstPage && !isFetching) {
      page--
      fetchFiles(undefined, undefined, () => openMediaAt(mediaFiles.length - 1, -1))
    }
    return
  }
  ...
  if (index >= mediaFiles.length) {
    if (!isLastPage && !isFetching) {
      page++
      fetchFiles(undefined, undefined, () => openMediaAt(0, 1))
    }
    return
  }
...
function addCategoryFormListener() {
  const form = document.getElementById('file-category-form')

  form.addEventListener('submit', event => {
    event.preventDefault()

    const categoryInput = document.getElementById('category').value.trim()
    const subcategoryInput = document.getElementById('subcategory').value.trim()

    // CSV → array, remove empties
    const tags = subcategoryInput
      .split(',')
      .map(t => t.trim())
      .filter(Boolean)

    // Reset paging when doing a new search
    page = 0

    fetchFiles(categoryInput, tags)
  })
}
